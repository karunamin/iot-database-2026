...
- 지식 Knowlege : 정보를 통한 사무리안 현상에 대한 이해

### 데이터베이스 DataBase

- 조직에 필요한 정보를 얻기 위해서 논리적으로 연관된 데이터를 구조적으로 통합, 저장해 놓은 것
...
- Docker - 애플리케이션 신속구축, 테스트, 서비스할 수 있는 컨테이너 기반의 가상화 플랫폼
...
1. 도커설치
    ...
    - Linux용 Windows 하위시스템 설치 필수, `wsl --update` 실행
...
- Structured Query Langeage
    - 구조화된 질의 언어
    - 데이터베이스에서 데이터를 조작하고, 테이블과 같은 객체를 컨트롤하는 등의 작업을 수행하는 프로그래밍 언어

- SQL 종류
    - Data Manipulation Language - 데이터 조작 언어. `SELECT`, `INSERT`, `UPDATE`, `DELETE` 와 같은 데이터를 조갖하는 언어.
    - Data Definition Language - 데이터 정의어. `CREATE`, `ALTER`, `RENAME`, `DROP` 같은 객체(데이터베이스, 테이블, 사용자, 뷰, 인덱스,... )를 처리하는 언어.
    - Data Control Language - 데이터 `GRANT`, `REVOKE` 와 같이 사용자에게 권한주고 해제하는 기능을 처리하는 언어.
...
- [쿼리](./day%2005/1.Sakila_practice.sql)

### 뷰

- VIEW - [쿼리](./day%2005/2.VIEW.sql)
...
- 뷰 특정
    ...
    - 뷰는 보기위해서 생성하므로 SELECT 이외 DML은 거의 사용하지 않음
...
- `인덱스 주의사항`
    ...
    - 인덱스마다 ASC, DESC로 정렬해야 하기 때문에 부가적인 처리가 많아짐
...
- Common Table Expression : 공통으로 쓸 수 있는 테이블 표현기법 - [쿼리](./day%2005/4.CTE.sql)
...
- TCL
    - Transaction Control Language에 포함된 `START TRANSACTION`, `COMIT`, `ROLLBACK`, `SAVEPOINT` 학습
...
-  계좌이체 예시 - A가 B에게 100만원 보낸다.
    - A의 계좌에서 100만원 차감
    - B의 계좌에 100만원 추가
    - 1번만 실행되고 2번이 실해하면, 돈이 사라짐(x)
    - 2번만만 실행되고 1번이 실패하면, 돈이 복사됨(x)

- 트랜잭션 4가지 특징 (`ACID`)
    - 원자성(Atomicity) : 전부 성공 OR 전부 실패(All or Nothing), 중간상태 없음
    - 일과성(Consistency) : 거래 전후로 데이터 규칙이 유지됨, 전체 합은 변경없음
...
- 환경설정 > 연결 > 연결유형 아래 `Auto-commit by default` 체크해제 -> 트랜잭션 사용모드
...
- `격리수준` - 동시 여러 트랜잭션이 실행될때 서로의 데이터에 얼마나 영향을 줄지 제어하는 기준
    - 최하 - Read Uncommitted. 커밋되지 않은 데이터 읽을 수 있음
    - 중간 - Read Committed. 커밋된 데이터만 읽음
    - 기본 - `Repeatable Read`. MySQL기본값. 같은 트랜잭션 안에서는 항상 같은결과
    - 최고 - Serializable. 순차적 실행. 동시성 거의 없음. 안전하지만 성능 최악

- 동시성 제어문제 
    - Dirty Read - 다른 트랜잭션이 아직 커밋하지 않은 데이터를 읽는 현상
    - Nom-repeatable Read - 같은 트랙잰션 안에서 같은 데이터를 두번 읽었을때 결과가 다른 현상
...
- 트랜잭션 확인 쿼리(관리자용)
    ```sql
    SELECT *FROM imformation_schema.INNODB_TRX it;
    ```
...
- 생산
    - DBeaver에서 해당 DB Procedure 폴더에서 마우스 오른쪽 버튼 > Create New Proceder
    - Name, 필요한 함수명 입력
    - Type, FUNCTION 선택
...
- 저장 프로시저 [쿼리1](./day07/1.PROCEDURE_원본l.sql), [쿼리2](./day07/1.PROCEDURE_실행.sql)
    - 함수와 달리 리턴값이 없음
    - 알반 쿼리문에 포함불가
...
- Trigger - [쿼리1](./day07/2.TRIGGER.sql), [쿼리](./day07/2.TRIGGER_원형.sql)
...
- 속성(Attribute)
    - 개체가 가지는 성질
    - 일반 속성(타원), 키 속성(글자에 밑줄), 다중속성(이중타원), 유도석성(점선타원),....
...
- 학원 수강관리 시스템 - [쿼리1](./day08/1.ERD_DATABASE_USER생성쿼리.sql), [쿼리2](./day08/2.ERD_테이블생성쿼리.sql)
    - 학생의 학원(대학)에서의 해당 강사에게 속한 과목을 수강신청하는 시스템 DB설계
...
- **제1정규형**(1NF)
    - 속성값이 원자갑이어야 함. 한 컬럼에 여러값이 들어가면 안됨
...
- MySQL 데이터베이스화
    ...
    - 학원 수강신청 시스템(Institude Enrollment System) -> PA-IES -> Pass
    ...
    - 왼쪽은 강사정보가 없어도 입력 가능, 오른쪽은 강사번호 입력 필수
...
```text
-> Sort: orders_big.order_date DESC  (cost=1.04e+6 rows=9.96e+6) (actual time=2641..2641 rows=26 loops=1)
    -> Filter: (orders_big.customer_id = 123456)  (cost=1.04e+6 rows=9.96e+6) (actual time=1167..2641 rows=26 loops=1)
        -> Table scan on orders_big  (cost=1.04e+6 rows=9.96e+6) (actual time=0.0974..2351 rows=10e+6 loops=1)
```
...
```text
-> Index lookup on orders_big using idx_orders_customer_id_and_order_date (customer_id=123456) (reverse)  (cost=28.6 rows=26) (actual time=2.41..10.1 rows=26 loops=1)
```
...
- MySQL 연동 순서 - [소스](./day09/MySqlMadangApp/main.cpp)
    1. 콘솔, 인코딩 UTF-8 설정
    2. 연결, 행데이터, 결과 구조체 변수, 포인터 선언
    3. MySQL 초기화
    4. 접속정보로 접속
    5. 서버 문자셋 확인 설정
    6. 쿼리 실행
    7. 결과 메모리 저장
    8. 한 행씩 Fetch, 출력(setlect에 한함)
    9. 결과 메모리 해제
    10. 접속 종료
